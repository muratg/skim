...
/*---[PATCHABLESECTION]---*/
    ...
    const patchRef = (initialValue) => {
      let $value = immutable(initialValue);
      const getValue = () => $value;
      const setValue = (newValue) => ($value = immutable(newValue));
      const mutateValue = (updateFunc) => ($value = patched($value, updateFunc));
      return [getValue, setValue, mutateValue];
    };

    const immutable = (x) => produce(x, (a) => a);

    const patched = (x, f) => produce(x, (xx) => void f(xx));

    const patching = (x) => (f) => produce(x, (xx) => void f(xx));

    const patcher = (f) => (x) => produce(x, (xx) => void f(xx));

    return { immutable, patchRef, patched, patching, patcher };
...
Object.assign(
  ...
  ((USING = typeof window === "object" ? (_) => window : (_) => module.exports) => {
    ...
    const { immutable } = USING("patchable");
    ...
    const range = (start, end = null) => {
      ...
      return immutable(new Array(end - start).fill(undefined).map((_, i) => i + start));
    };
    const first = (x) => immutable(x.slice(0, 1)[0]);
    const last = (x) => immutable(x.slice(-1)[0]);
    const rest = (x) => immutable(x.slice(1));
    const butlast = (x) => immutable(x.slice(0, -1));
    const concat = (x, y) => immutable(x.concat(y));

    // HOF
    const map = (f, l) => immutable(l.map((x) => f(x)));
    const mapi = (f, l) => immutable(l.map((x, i) => f(x, i)));
    const filter = (f, l) => immutable(l.filter((x) => f(x)));
    const filteri = (f, l) => immutable(l.filteri((x, i) => f(x, i)));
    const reduce = (f, acc, l) => immutable(l.reduce(f, acc)); // TODO simplify for 2 param only
    const reduceRight = (f, acc, l) => immutable(l.reduceRight(f, acc)); // TODO simplify for 2 param only

    // String (string's are by themselves immutable, so no "patchable" wrapping necessary)
    const stringMatch = (str, rx) => str.match(rx);
    const stringMatchAll = (str, rx) => str.matchall(rx);
    const stringJoin = (lst, sep = " ") => lst.join(sep);
    const stringSplit = (str, sep = " ") => str.split(sep);
    const stringReplace = (str, a, b) => str.replace(a, b);
    const stringUpper = (str) => str.toUpperCase();
    const stringLower = (str) => str.toLowerCase();
    const stringLocaleUpper = (str) => str.toLocaleUpperCase();
    const stringLocaleLower = (str) => str.toLocaleLowerCase();

    // Logic
    const truthiness = (v) => !(v == undefined || v == null || v === false);

    // Utils w/ side effects
    const raise$ = (...err) => {
      throw new Error(err.join(" "));
    };
    const assert$ = (tst, ...err) => {
      if (!truthiness(tst)) {
        raise$(...err);
      }
    };
    const log$ = (...things) => console.log(...things);
    const warn$ = (...things) => console.warn(...things);
