...
Object.assign(
  ...
  ((USING = typeof window === "object" ? (_) => window : (lib) => module.exports) => {
    ("use strict");
    ...
    function revokeDraft(draft) {
      ...
      if (state.type_ === ProxyTypeProxyObject || state.type_ === ProxyTypeProxyArray) state.revoke_();
    ...
    function finalize(rootScope, value, path) {
      ...
      if (!state.finalized_) {
        ...
        var result = state.type_ === ProxyTypeES5Object || state.type_ === ProxyTypeES5Array ? (state.copy_ = shallowCopy(state.draft_, true)) : state.copy_; // For ES5, create a good copy from the draft first, with added keys and without deleted keys. // finalize all children of the copy
    ...
    const $$produce$$ = produce; // USING("immer").produce;

    const patchRef = (initialValue) => {
      let $value = patchable(initialValue);
      const getValue = () => $value;
      const setValue = (newValue) => ($value = patchable(newValue));
      const mutateValue = (updateFunc) => ($value = patched($value, updateFunc));
      return [getValue, setValue, mutateValue];
    };

    const patchable = (x) => $$produce$$(x, (a) => a);
    ...
    return { patchable, patchRef, patched, patching, patcher };
