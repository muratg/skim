...
test("compile basic expressions", () => {
  let res = compile_basic("(* 1 2)");
  process.stdout.write("" + res);
  expect(res).toEqual("1 * 2;\n");
  expect(compile_basic("(/ 10.2 -3)")).toEqual("10.2 / -3;\n");
  expect(compile_basic("(* 111 (- 2 20))")).toEqual("111 * (2 - 20);\n");
});
...
test("compile quote", () => {
  expect(compile_basic("(quote 1)")).toEqual("1;\n");
  expect(compile_basic('(quote "x")')).toEqual('"x";\n');
  expect(compile_basic('(quasiquote "x")')).toEqual('"x";\n');
});
